...
install-local:
	...
	go build -o $(STEAMPIPE_INSTALL_DIR)/plugins/local/steampipe-plugin-cortex.plugin -tags "${BUILD_TAGS}" *.go
	go build -o $(STEAMPIPE_INSTALL_DIR)/plugins/local/steampipe-plugin-cortex/steampipe-plugin-cortex.plugin -tags "${BUILD_TAGS}" *.go
test: install-local
